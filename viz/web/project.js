// Project: one converted iRIC result (a Zarr store served at data/<name>/) plus its wasm-side
// buffers and analysis products.
...
export class Project {
  ...
  static async fromGroup(name, root) {
    const p = new Project();
    p.name = name; p.url = root.url || null;
  ...
  get(key, t) { return this.arrays[key].getChunk([t, 0, 0]); }
  ...
  addDerived(key, label, data, unit, cmap) {
    const finite = []; for (const v of data) if (Number.isFinite(v)) finite.push(v);
    let lo = Infinity, hi = -Infinity; for (const v of finite) { if (v < lo) lo = v; if (v > hi) hi = v; }
    this.arrays[key] = { attrs: { min: lo, max: hi, original_name: label }, getChunk: async () => data, stats: { fetched: 0, bytes: 0, fetchMs: 0, decodeMs: 0 }, cache: new Map(), cmap };
  ...
  /** Bed elevation + water surface (+ an extra variable) along an i- or j-line at step t. */
  ...
  /** Whole-run analysis: wet area / volume / max depth / max speed per step, arrival time, duration. */
  async runAnalysis(thr, onProgress) {
    const { N, ni, nj, nt, timeArr, ptr } = this;
    ensureScratch(N);
    f32(ptr.arr, N).fill(NaN); f32(ptr.dur, N).fill(0);
    const series = { area: [], vol: [], maxd: [], maxv: [] };
    for (let t = 0; t < nt; t++) {
    ...
    const arrival = Float32Array.from(f32(ptr.arr, N), (v) => v / 60);
    const duration = Float32Array.from(f32(ptr.dur, N), (v) => v / 60);
